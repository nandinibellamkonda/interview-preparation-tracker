...
// Create or Update Java Topic Progress
exports.createOrUpdateTopic = async (req, res) => {
  try {
    const { topicName, progressPercentage, confidenceRating, notes } = req.body;

    let topic = await JavaTopic.findOne({
      userId: req.userId,
      topicName,
    });

    if (topic) {
      topic = await JavaTopic.findByIdAndUpdate(
        topic._id,
        {
          progressPercentage,
          confidenceRating,
...
exports.getTopics = async (req, res) => {
  try {
    const topics = await JavaTopic.find({ userId: req.userId }).sort({ createdAt: -1 });
...
exports.getTopicById = async (req, res) => {
  try {
    const topic = await JavaTopic.findById(req.params.id);
...
exports.updateTopic = async (req, res) => {
  try {
    let topic = await JavaTopic.findById(req.params.id);
    ...
    topic = await JavaTopic.findByIdAndUpdate(req.params.id, req.body, {
      new: true,
      runValidators: true,
    });
...
exports.deleteTopic = async (req, res) => {
  try {
    ...
    await JavaTopic.findByIdAndDelete(req.params.id);
...
exports.getProgressSummary = async (req, res) => {
  try {
    const topics = await JavaTopic.find({ userId: req.userId });
