...
(async ()=>{
  ...
  try{
    ...
    // List companies
    let list = await fetch(base + '/api/companies', { headers: ah() });
    out.list = { status: list.status, body: await list.text().then(t=>t.slice(0,2000)) };

    // Update round: clear OA
    if (createdCompany) {
      let up = await fetch(base + '/api/companies/' + createdCompany._id + '/round-status', { method:'PUT', headers:ah(), body: JSON.stringify({ round:'oaStatus', status:'Cleared' }) });
      out.oa_cleared = { status: up.status, body: await up.text() };

      // Mark technicalRound1 as Rejected
      ...
      out.tech1_rejected = { status: rej.status, body: await rej.text() };

      // Create second company and mark all cleared to simulate offer
      let create2 = await fetch(base + '/api/companies', { method:'POST', headers:ah(), body: JSON.stringify({ companyName:`OfferCo-${TEST_ID}`, role:'SWE', applicationDate: new Date().toISOString(), notes:'Offer test', oaStatus:'Cleared', technicalRound1Status:'Cleared', technicalRound2Status:'Cleared', hrRoundStatus:'Cleared' }) });
      out.create2 = { status: create2.status, body: await create2.text() };
      const created2 = create2.status===201 ? JSON.parse(out.create2.body) : null;
      if (created2) created.companies.push(created2._id);
    }
    ...
    try {
      ...
      // Attempt to delete test user directly
      try {
