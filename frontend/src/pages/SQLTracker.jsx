...
import { sqlService } from '../services/api.js';
import { Database, Plus, Pencil, Trash2, BarChart3, BookOpen } from 'lucide-react';

const topicOptions = [
  'Constraints', 'Keys', 'Joins', 'Subqueries', 'Normalization', 'Views', 'Indexes', 'Stored Procedures', 'Triggers', 'Transactions'
];
...
const SQLTracker = () => {
  ...
  const loadTopics = async () => {
    try {
      setLoading(true);
      const response = await sqlService.getTopics(user?.token);
      setTopics(response.topics || []);
    } catch (err) {
      setError(err.message || 'Unable to load SQL tracker');
    } finally {
      setLoading(false);
    }
  };
  ...
  const saveTopic = async () => {
    ...
    try {
      ...
      await sqlService.createOrUpdateTopic(user?.token, payload);
      await loadTopics();
      resetForm();
    } catch (err) {
      setError(err.message || 'Unable to save SQL topic');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Delete this SQL topic?')) return;
    try {
      setSaving(true);
      await sqlService.deleteTopic(undefined, id);
      await loadTopics();
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
        <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            <p className="text-sm uppercase tracking-[0.32em] text-slate-500">SQL Tracker</p>
            <h1 className="mt-2 text-3xl font-semibold text-white">SQL topics, confidence and notes</h1>
            <p className="mt-2 text-slate-400 max-w-2xl">Track key SQL concepts and update your progress on queries, joins, and transactions.</p>
          </div>
        ...
        <div className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
          <div className="rounded-3xl border border-slate-800 bg-[#0F172A] p-6">
            <div className="flex items-center justify-between mb-6">
              <div>
                <h2 className="text-xl font-semibold text-white">Update SQL mastery</h2>
                <p className="text-sm text-slate-500">Add notes and progress for each SQL concept.</p>
              </div>
            ...
            <div className="grid gap-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Add a quick explanation or SQL pattern to revisit"
                />
              </div>
          ...
          <div className="rounded-3xl border border-slate-800 bg-[#0F172A] p-6">
            <div className="flex items-center gap-3 mb-5">
              ...
              <div>
                <h2 className="text-lg font-semibold text-white">SQL Progress</h2>
                <p className="text-sm text-slate-500">Live readiness for SQL concepts.</p>
              </div>
            </div>
            <div className="grid gap-4">
        ...
        <div className="rounded-3xl border border-slate-800 bg-[#0F172A] p-6">
          <div className="flex items-center justify-between mb-5">
            <div>
              <h2 className="text-xl font-semibold text-white">SQL Topic List</h2>
              <p className="text-sm text-slate-500">Review each saved SQL concept and edit details.</p>
            </div>
            <Database className="h-5 w-5 text-slate-400" />
          </div>
          {loading ? (
            <p className="text-slate-500">Loading SQL topics...</p>
          ) : topics.length === 0 ? (
            <p className="text-slate-500">No SQL topics saved yet. Add one to get started.</p>
          ) : (
              ...
              {topics.map((topic) => (
                <div key={topic._id} className="rounded-3xl border border-slate-800 bg-[#121A2D] p-4">
                  ...
                  <div className="mt-4 rounded-3xl bg-[#0F172A] p-4 text-sm text-slate-300">
                    <p>{topic.notes || 'No notes available for this topic yet.'}</p>
                  </div>
                </div>
              ))}
            </div>
